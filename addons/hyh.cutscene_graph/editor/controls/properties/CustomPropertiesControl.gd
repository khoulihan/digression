...
@onready var AddButton = $VB/AddButton
@onready var PropertiesContainer = $VB/PropertiesContainer
...
func add_property(property: Dictionary) -> void:
	...
	PropertiesContainer.add_child(new_property)
...
func _property_select_dialog_cancelled(dialog) -> void:
	get_tree().root.remove_child(dialog)
	dialog.queue_free()


func _property_select_dialog_selected(property, dialog) -> void:
	get_tree().root.remove_child(dialog)
	dialog.queue_free()
...
func _on_property_remove_requested(property_name) -> void:
	...
	dialog.initial_position = Window.WINDOW_INITIAL_POSITION_CENTER_MAIN_WINDOW_SCREEN
	dialog.canceled.connect(_property_remove_canceled.bind(dialog))
	dialog.confirmed.connect(_property_remove_confirmed.bind(property_name, dialog))
	get_tree().root.add_child(dialog)
	dialog.popup()


func _property_remove_canceled(dialog) -> void:
	get_tree().root.remove_child(dialog)
	dialog.queue_free()


func _property_remove_confirmed(property_name, dialog) -> void:
	get_tree().root.remove_child(dialog)
	dialog.queue_free()
